...
(defun cubemap->resources (name px-image nx-image py-image ny-image pz-image nz-image)
  (let* ((images (list px-image nx-image py-image ny-image pz-image nz-image))
    ...
    (list (notalone-thriced::make-pixel-buffer-resource pixel-buffer-name
                                      `((:ubyte ,channels))
                                      total-data
                                      total-size)
          (notalone-thriced::make-texture-resource name
                                 width
                                 height
                                 (ecase channels
                                   (1 :r8)
...
(defun vertex-buffer->resources (vbuf name)
  (list (notalone-thriced::make-vertex-buffer-resource name
...
(defun index-buffer->resources (ibuf name)
  (list (notalone-thriced::make-index-buffer-resource name
